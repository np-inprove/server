...
func (mq *MilestoneQuery) QueryStudyPlan() *StudyPlanQuery {
	query := (&StudyPlanClient{config: mq.config}).Query()
...
func (mq *MilestoneQuery) Clone() *MilestoneQuery {
	...
	return &MilestoneQuery{
		...
		withStudyPlan: mq.withStudyPlan.Clone(),
...
// WithStudyPlan tells the query-builder to eager-load the nodes that are connected to
// the "study_plan" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MilestoneQuery) WithStudyPlan(opts ...func(*StudyPlanQuery)) *MilestoneQuery {
	query := (&StudyPlanClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withStudyPlan = query
	return mq
}
...
func (mq *MilestoneQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Milestone, error) {
	var (
		...
		_spec       = mq.querySpec()
		loadedTypes = [1]bool{
			mq.withStudyPlan != nil,
		}
	)
	if mq.withStudyPlan != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withStudyPlan; query != nil {
		if err := mq.loadStudyPlan(ctx, query, nodes, nil,
			func(n *Milestone, e *StudyPlan) { n.Edges.StudyPlan = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MilestoneQuery) loadStudyPlan(ctx context.Context, query *StudyPlanQuery, nodes []*Milestone, init func(*Milestone), assign func(*Milestone, *StudyPlan)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Milestone)
	for i := range nodes {
		if nodes[i].study_plan_milestones == nil {
			continue
		}
		fk := *nodes[i].study_plan_milestones
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "study_plan_milestones" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
