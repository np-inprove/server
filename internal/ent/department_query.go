...
func (dq *DepartmentQuery) QueryInstitution() *InstitutionQuery {
	query := (&InstitutionClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, department.InstitutionTable, department.InstitutionColumn),
		)
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withInstitution: dq.withInstitution.Clone(),
...
func (dq *DepartmentQuery) WithInstitution(opts ...func(*InstitutionQuery)) *DepartmentQuery {
	query := (&InstitutionClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withInstitution = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [4]bool{
			dq.withInstitution != nil,
			dq.withParent != nil,
			dq.withChildren != nil,
			dq.withUsers != nil,
		}
	)
	if dq.withInstitution != nil || dq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withInstitution; query != nil {
		if err := dq.loadInstitution(ctx, query, nodes, nil,
			func(n *Department, e *Institution) { n.Edges.Institution = e }); err != nil {
			return nil, err
		}
	}
...
func (dq *DepartmentQuery) loadInstitution(ctx context.Context, query *InstitutionQuery, nodes []*Department, init func(*Department), assign func(*Department, *Institution)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Department)
	for i := range nodes {
		if nodes[i].institution_departments == nil {
			continue
		}
		fk := *nodes[i].institution_departments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "institution_departments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DepartmentQuery) loadParent(ctx context.Context, query *DepartmentQuery, nodes []*Department, init func(*Department), assign func(*Department, *Department)) error {
	...
	nodeids := make(map[int][]*Department)
	for i := range nodes {
		if nodes[i].department_children == nil {
			continue
		}
		fk := *nodes[i].department_children
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_children" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DepartmentQuery) loadChildren(ctx context.Context, query *DepartmentQuery, nodes []*Department, init func(*Department), assign func(*Department, *Department)) error {
	...
	for _, n := range neighbors {
		fk := n.department_children
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_children" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
