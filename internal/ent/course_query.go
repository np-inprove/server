...
type CourseQuery struct {
	...
	withAcademicSchool *AcademicSchoolQuery
	withFKs            bool
...
// QueryAcademicSchool chains the current query on the "academic_school" edge.
func (cq *CourseQuery) QueryAcademicSchool() *AcademicSchoolQuery {
	query := (&AcademicSchoolClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CourseQuery) Clone() *CourseQuery {
	...
	return &CourseQuery{
		...
		withAcademicSchool: cq.withAcademicSchool.Clone(),
...
// WithAcademicSchool tells the query-builder to eager-load the nodes that are connected to
// the "academic_school" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CourseQuery) WithAcademicSchool(opts ...func(*AcademicSchoolQuery)) *CourseQuery {
	query := (&AcademicSchoolClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withAcademicSchool = query
	return cq
}
...
func (cq *CourseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Course, error) {
	var (
		...
		loadedTypes = [2]bool{
			cq.withStudents != nil,
			cq.withAcademicSchool != nil,
		}
	)
	if cq.withAcademicSchool != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withAcademicSchool; query != nil {
		if err := cq.loadAcademicSchool(ctx, query, nodes, nil,
			func(n *Course, e *AcademicSchool) { n.Edges.AcademicSchool = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CourseQuery) loadStudents(ctx context.Context, query *UserQuery, nodes []*Course, init func(*Course), assign func(*Course, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(course.StudentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.course_students
		if fk == nil {
			return fmt.Errorf(`foreign-key "course_students" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "course_students" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CourseQuery) loadAcademicSchool(ctx context.Context, query *AcademicSchoolQuery, nodes []*Course, init func(*Course), assign func(*Course, *AcademicSchool)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Course)
	for i := range nodes {
		if nodes[i].academic_school_courses == nil {
			continue
		}
		fk := *nodes[i].academic_school_courses
	...
	query.Where(academicschool.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "academic_school_courses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
