...
func (pu *PetUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := pu.mutation.SvgRaw(); ok {
	...
	if pu.mutation.OwnerCleared() {
		...
		createE := &UserPetCreate{config: pu.config, mutation: newUserPetMutation(pu.config, OpCreate)}
		createE.defaults()
...
func (puo *PetUpdateOne) sqlSave(ctx context.Context) (_node *Pet, err error) {
	...
	if value, ok := puo.mutation.SvgRaw(); ok {
		_spec.SetField(pet.FieldSvgRaw, field.TypeString, value)
	}
	if puo.mutation.OwnerCleared() {
		...
		createE := &UserPetCreate{config: puo.config, mutation: newUserPetMutation(puo.config, OpCreate)}
		createE.defaults()
		_, specE := createE.createSpec()
		edge.Target.Fields = specE.Fields
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
