...
type UserQuery struct {
	...
	withVotedDeadlines    *DeadlineQuery
	withAuthoredDeadlines *DeadlineQuery
...
func (uq *UserQuery) QueryVotedDeadlines() *DeadlineQuery {
	query := (&DeadlineClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withVotedDeadlines:    uq.withVotedDeadlines.Clone(),
		withAuthoredDeadlines: uq.withAuthoredDeadlines.Clone(),
...
// WithVotedDeadlines tells the query-builder to eager-load the nodes that are connected to
// the "voted_deadlines" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithVotedDeadlines(opts ...func(*DeadlineQuery)) *UserQuery {
	query := (&DeadlineClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withVotedDeadlines = query
	return uq
}

// WithAuthoredDeadlines tells the query-builder to eager-load the nodes that are connected to
// the "authored_deadlines" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAuthoredDeadlines(opts ...func(*DeadlineQuery)) *UserQuery {
	query := (&DeadlineClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAuthoredDeadlines = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [12]bool{
			...
			uq.withVotedDeadlines != nil,
			uq.withAuthoredDeadlines != nil,
			uq.withUserPets != nil,
	...
	if query := uq.withVotedDeadlines; query != nil {
		if err := uq.loadVotedDeadlines(ctx, query, nodes,
			func(n *User) { n.Edges.VotedDeadlines = []*Deadline{} },
			func(n *User, e *Deadline) { n.Edges.VotedDeadlines = append(n.Edges.VotedDeadlines, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withAuthoredDeadlines; query != nil {
		if err := uq.loadAuthoredDeadlines(ctx, query, nodes,
			func(n *User) { n.Edges.AuthoredDeadlines = []*Deadline{} },
			func(n *User, e *Deadline) { n.Edges.AuthoredDeadlines = append(n.Edges.AuthoredDeadlines, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUserPets; query != nil {
		if err := uq.loadUserPets(ctx, query, nodes,
			...
			func(n *User, e *UserPet) { n.Edges.UserPets = append(n.Edges.UserPets, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadRedemptions(ctx context.Context, query *RedemptionQuery, nodes []*User, init func(*User), assign func(*User, *Redemption)) error {
	...
	query.Where(predicate.Redemption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RedemptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.redemption_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "redemption_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadVotedDeadlines(ctx context.Context, query *DeadlineQuery, nodes []*User, init func(*User), assign func(*User, *Deadline)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.VotedDeadlinesTable)
		s.Join(joinT).On(s.C(deadline.FieldID), joinT.C(user.VotedDeadlinesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.VotedDeadlinesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.VotedDeadlinesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Deadline](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "voted_deadlines" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadAuthoredDeadlines(ctx context.Context, query *DeadlineQuery, nodes []*User, init func(*User), assign func(*User, *Deadline)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Deadline(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuthoredDeadlinesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.deadline_author
		if fk == nil {
			return fmt.Errorf(`foreign-key "deadline_author" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "deadline_author" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadUserPets(ctx context.Context, query *UserPetQuery, nodes []*User, init func(*User), assign func(*User, *UserPet)) error {
	...
	query.Where(predicate.UserPet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserPetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadGroupUsers(ctx context.Context, query *GroupUserQuery, nodes []*User, init func(*User), assign func(*User, *GroupUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupuser.FieldUserID)
	}
