...
func (pq *PetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(pet.OwnerPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(pet.OwnerPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(pet.OwnerPrimaryKey[0]))
...
func (pq *PetQuery) loadUserPet(ctx context.Context, query *UserPetQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *UserPet)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "pet_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
